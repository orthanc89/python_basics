...
class chatbot():
    ...
    def chat_ai(self, eingabe):
        ...
        if xindex != -1:
            ...
            #print("Antwort" + out)
        return out
    def genrandom_answer(self, random_antworten_lst):
        xindex = -1
        return xindex
    def get_random_answer(self, xindex,random_antworten_lst):
        out = ""
        return out
    def check_benutzereingabe(self, eingabe):
        if len(eingabe) <= 1:
            x = False
        else:
            x = True
        return x
    def show_end(self, winke="bye bye"):
        print(winke)

    def check_frage(self, eingabe,xlist):
        out = -1
    ...
    def get_answer(self,xindex,anworten_lst):
        out = " "
